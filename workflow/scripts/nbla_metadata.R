...
read_picocyano <- function(file_name) {
    ...
    read.csv(file_name) %>%
        mutate(organism = paste(Species, Name)) %>%
        select(genome = Name, organism, SubCluster, Clade, SubClade) %>%
...
genomes <- read.jtree(jtree_file) %>%
    ...
    select(genome = Accession, nbla_category2, nbla_category, alias = Phage, organism = Phage)
